...
describe('Integration Tests - Restaurant Search Flow', () => {
  ...
  describe('Complete Search and Recommendation Flow', () => {
    test('should complete full flow within performance targets', async () => {
      ...
      const mockDetailsResponse = (placeId: string, index: number) => ({
        data: {
          ...
          result: {
            ...
            types: ['restaurant', index % 2 === 0 ? 'italian_restaurant' : 'japanese_restaurant'],
      ...
      const restaurants = await googleMapsService.searchRestaurants(searchParams);
      const searchDuration = Date.now() - startTime;

      // Step 2: Get recommendations
      const recommendationStartTime = Date.now();
      const recommendations = await recommendationService.getRecommendations(restaurants, searchParams);
      ...
      for (let i = 1; i < restaurants.length; i++) {
        expect(restaurants[i].distance || 0).toBeGreaterThanOrEqual(restaurants[i - 1].distance || 0);
      }
    ...
    test('should handle booking flow integration', async () => {
      ...
      const startTime = Date.now();
      const bookingResult = await bookingService.makeReservation(bookingRequest);
      const bookingInstructions = await bookingService.getBookingInstructions(mockRestaurant);
      const duration = Date.now() - startTime;
      ...
      expect(bookingResult.confirmationDetails).toContain('Bookable Restaurant');
  ...
  describe('Error Handling Integration', () => {
    test('should gracefully handle API failures', async () => {
      const mockClient = {
        placesNearby: jest.fn().mockRejectedValue(new Error('API temporarily unavailable')),
        placeDetails: jest.fn().mockRejectedValue(new Error('API temporarily unavailable'))
      };
      ...
      await expect(googleMapsService.searchRestaurants(searchParams))
        .rejects
        .toThrow('Failed to search restaurants');
  ...
  describe('Caching Integration', () => {
    test('should utilize caching across service calls', async () => {
      ...
      // First search
      const firstResults = await googleMapsService.searchRestaurants(searchParams);
      expect(mockClient.placeDetails).toHaveBeenCalledTimes(1);

      // Second search - should use cached data
      const startTime = Date.now();
      const secondResults = await googleMapsService.searchRestaurants(searchParams);
      const duration = Date.now() - startTime;
  ...
  describe('Memory and Resource Management', () => {
    test('should not leak memory with repeated operations', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        const searchParams: RestaurantSearchParams = {
          location: { latitude: 37.7749 + i * 0.001, longitude: -122.4194 + i * 0.001 },
          cuisineTypes: ['Italian'],
          mood: 'casual',
          event: 'gathering'
        };
        ...
        const restaurants = await googleMapsService.searchRestaurants(searchParams);
        await recommendationService.getRecommendations(restaurants, searchParams);
      }
