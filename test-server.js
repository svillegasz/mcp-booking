...
async function testServer() {
  ...
  try {
    ...
    console.log(
      "📤 Sending request:",
      JSON.stringify(searchRequestExplicit, null, 2)
    );
    ...
    console.log(
      "📤 Sending request:",
      JSON.stringify(hotpotSearchRequest, null, 2)
    );
    ...
    console.log(
      "📤 Sending request:",
      JSON.stringify(budgetSearchRequest, null, 2)
    );
    ...
    console.log(
      "📤 Sending request:",
      JSON.stringify(placeNameSearchRequest, null, 2)
    );
    server.stdin.write(JSON.stringify(placeNameSearchRequest) + "\n");
    await new Promise((resolve) => setTimeout(resolve, 8000)); // Wait for API call
    console.log("✅ Place name search completed");

    // Test 7: Test strict 2km radius filtering
    console.log("\n🔧 Test 7: Testing strict 2km radius filtering...");
    const radiusTestRequest = {
      jsonrpc: "2.0",
      id: 7,
      method: "tools/call",
      params: {
    ...
    console.log(
      "📤 Sending request:",
      JSON.stringify(radiusTestRequest, null, 2)
    );
    server.stdin.write(JSON.stringify(radiusTestRequest) + "\n");
    await new Promise((resolve) => setTimeout(resolve, 8000)); // Wait for API call
    console.log("✅ 2km radius test completed");
          ...
          if (
            ...
            try {
              ...
              if (data.recommendations) {
                ...
                console.log(
                  `📍 Search Location: ${data.searchCriteria.location.latitude}, ${data.searchCriteria.location.longitude}`
                );
                console.log(
                  `🍽️  Cuisine Types: ${data.searchCriteria.cuisineTypes.join(
                    ", "
                  )}`
                );
                ...
                if (data.searchCriteria.priceLevel) {
                  ...
                  console.log(
                    `💰 Price Level Filter: ${
                      data.searchCriteria.priceLevel
                    } (${priceLevelDesc[data.searchCriteria.priceLevel]})`
                  );
                }
                ...
                data.recommendations.forEach((rec, index) => {
                  ...
                  console.log(
                    `   ⭐ ${rec.restaurant.rating}/5 (${rec.restaurant.userRatingsTotal} reviews)`
                  );
                  ...
                  if (rec.restaurant.priceLevel) {
                    ...
                    console.log(
                      `   💰 Price Level: ${rec.restaurant.priceLevel} - ${
                        priceLevelDesc[rec.restaurant.priceLevel]
                      }`
                    );
                  } else {
                  ...
                  console.log(
                    `   🍽️  Cuisine: ${rec.restaurant.cuisineTypes.join(", ")}`
                  );
                  ...
                  if (rec.restaurant.bookingInfo) {
                    ...
                    console.log(
                      `      • Reservable: ${
                        rec.restaurant.bookingInfo.reservable ? "Yes" : "No"
                      }`
                    );
                    console.log(
                      `      • Online Booking: ${
                        rec.restaurant.bookingInfo.supportsOnlineBooking
                          ? "Yes"
                          : "No"
                      }`
                    );
                    console.log(
                      `      • Phone Required: ${
                        rec.restaurant.bookingInfo.requiresPhone ? "Yes" : "No"
                      }`
                    );

                    if (rec.restaurant.bookingInfo.bookingPlatform) {
                      console.log(
                        `      • Platform: ${rec.restaurant.bookingInfo.bookingPlatform}`
                      );
                    }

                    if (rec.restaurant.bookingInfo.bookingUrl) {
                      console.log(
                        `      • Booking URL: ${rec.restaurant.bookingInfo.bookingUrl}`
                      );
                    }
                  }
                ...
                console.log(
                  `📍 Location: ${data.searchCriteria.location.latitude}, ${data.searchCriteria.location.longitude}`
                );
                console.log(
                  `📊 Total Found: ${data.totalFound || 0} restaurants`
                );
                foundResults = true;
              }
            } catch (e) {
